...
const ApplicationStatus = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:8000/v1/applicationview');
...
// }
//     useEffect(() => {
//      const getloansinstallments = async () => {
//         try {
//             const response = await axios.get(`http://127.0.0.1:8000/v1/installments/${id}`);
//                 // get(response.data);
//                 setData(response.data);
//         } catch (error) {
//             console.error('Error fetching data:', error);
...
// }, []);
    ...
    return (
        <div className='table-responsive'>
            <h1 className="blockquote text-center">ALL APPLICATIONS AND STATUS</h1>
                ...
                <tbody>
                    {data.map(item => (
                        <tr key={item.id}>
                            ...
                            <td>{item.user}</td>
                           <td><button class="btn btn-outline-info me-2" type="button" style={buttonStyle}><NavLink className="nav-link " aria-current="page" to={`/installment/${item.id}`} >Check</NavLink></button></td> 
